...
!=====================================================================
  ...
  public:: Nudge_lon1, Nudge_lon2, Nudge_londelta
...
!  integer           Nudge_End_Day  ,Nudge_End_Sec
  ...
  logical::         NudgeQ_taperlon
  logical::         NudgeV_taperlon
  logical::         NudgeU_taperlon
  logical::         NudgeT_taperlon
  logical::         taper_invert
  real(r8)          Nudge_lat1
  real(r8)          Nudge_lat2
  real(r8)          Nudge_latdelta
  real(r8)          Nudge_lon1
  real(r8)          Nudge_lon2
  real(r8)          Nudge_londelta
...
contains
  ...
  subroutine nudging_readnl(nlfile)
   ...
   namelist /nudging_nl/ Nudge_Model,Nudge_Path,Nudge_Spec_Path,       &
                         ...
                         Nudge_lon1, Nudge_lon2, Nudge_londelta, &
                         NudgeQ_taper, NudgeU_taper, NudgeV_taper, NudgeT_taper, &
                         NudgeQ_taperlat, NudgeU_taperlat, NudgeV_taperlat, & 
                         NudgeT_taperlat , NudgeU_taperlon, NudgeV_taperlon, &
                         NudgeQ_taperlon, NudgeT_taperlon, taper_invert, &
                         Nudge_ZMLin
   ...
   NudgeV_taperlon=.false.
   NudgeU_taperlon=.false.
   ...
   if (masterproc) then
    ...
    write(iulog,*),'Nudge_lon1=',Nudge_lon1
    write(iulog,*),'Nudge_lon2=',Nudge_lon2
    write(iulog,*),'Nudge_londelta=',Nudge_londelta
    ...
    write(iulog,*),'NudgeQ_taperlon=',NudgeQ_taperlon
    write(iulog,*),'NudgeU_taperlon=',NudgeU_taperlon
    write(iulog,*),'NudgeV_taperlon=',NudgeV_taperlon
    write(iulog,*),'NudgeT_taperlon=',NudgeT_taperlon
    write(iulog,*),'taper_invert',taper_invert
   ...
   call mpibcast(Nudge_lon1     , 1, mpir8 , 0, mpicom)
   call mpibcast(Nudge_lon2     , 1, mpir8 , 0, mpicom)
   call mpibcast(Nudge_londelta , 1, mpir8 , 0, mpicom)
   ...
   call mpibcast(NudgeQ_taperlon, 1, mpilog, 0, mpicom)
   call mpibcast(NudgeU_taperlon, 1, mpilog, 0, mpicom)
   call mpibcast(NudgeV_taperlon, 1, mpilog, 0, mpicom)
   call mpibcast(NudgeT_taperlon, 1, mpilog, 0, mpicom)
   call mpibcast(taper_invert, 1, mpilog, 0, mpicom)
#endif
...
!!DIAG
    ...
    do lchnk=begchunk,endchunk
     ...
     do icol=1,ncol
       ...
       call nudging_set_profile(rlat,rlon,NudgeU_taper,NudgeU_taperlat,NudgeU_taperlon,taper_invert,Wprof,pver)
       Nudge_Utau(icol,:,lchnk)=Wprof(:)
       call nudging_set_profile(rlat,rlon,NudgeV_taper,NudgeV_taperlat,NudgeV_taperlon,taper_invert,Wprof,pver)
       Nudge_Vtau(icol,:,lchnk)=Wprof(:)
       call nudging_set_profile(rlat,rlon,NudgeT_taper,NudgeT_taperlat,NudgeT_taperlon,taper_invert,Wprof,pver)
       Nudge_Stau(icol,:,lchnk)=Wprof(:)
       call nudging_set_profile(rlat,rlon,NudgeQ_taper,NudgeQ_taperlat,NudgeQ_taperlon,taper_invert,Wprof,pver)
...
!   endif ! (Nudge_NDG_Refopt.ne.0) then
  ...
  !================================================================
  subroutine nudging_set_profile(rlat,rlon,ltaper,ltaperlat,ltaperlon,taper_invert,Wprof,nlev)
  !
...
!     write(iulog,*),'???rlat, Wprof=',rlat,' ',Wprof(5)
   ...
   if (ltaperlon) then
    if ((rlon.lt.(Nudge_lon1-Nudge_londelta)).or.(rlon.gt.(Nudge_lon2+Nudge_londelta))) then
        Wprof(:)=0.
    end if

    if (Nudge_londelta.gt.0) then
       if ((rlon.ge.(Nudge_lon1-Nudge_londelta)).and.(rlon.le.Nudge_lon1)) then
        Wprof(:)=Wprof(:)*(1./Nudge_londelta)*(rlon-(Nudge_lon1-Nudge_londelta))
       end if
       if ((rlon.gt.Nudge_lon2).and.(rlon.le.(Nudge_lon2+Nudge_londelta))) then
        Wprof(:)=Wprof(:)*(1./Nudge_londelta)*( (Nudge_lon2+Nudge_londelta)-rlon)
       end if
    end if
   end if

   if (taper_invert) then
      Wprof(:)=1.-Wprof(:)
   end if
