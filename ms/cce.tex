...
CCE monsoon precipitation was reduced by 4mm (tk, precise this) on average between the pre-industrial and present-day simulations, with another 9mm reduction expected with SSP3-7.0 by the end of the century.
Given that the 2020 monsoon failure represented a 64.4mm shortfall relative to climatological precipitation (Supp Figure 3), the CCE scenario effect of -4mm is small relative to the overall drought signal. As such, the meteorological drought seems primarily attributable to unforced climate variability \cite{seager2022}.
...
However, the CCE experiment cannot capture any coupled circulation feedbacks that could exacerbate or mitigate the radiative response. 
A tk sentence here providing a literature reference to forced circulation changes.


The CESM2-LE, which does include circulation feedbacks, likewise shows a drying response across the most drought years, but with significantly more noise (Figure \ref{fig:scatter}d-f).
Because the CCE constrains the large-scale circulation, there is much less variance within each ensemble. 
...
Precipitation seems to decline slightly from pre-industrial to present-day, but appears equivalent when comparing present-day and future. 
...
The CCE simulations vary only by the initial conditions, whereas the CESM2-LE data are sampled across various initial conditions and circulations patterns (with strong covariance). 
...
This would seem to indicate that coupled circulation feedbacks may partially mitigate extreme divergence events in CESM2, under late-century SSP3-7.0 conditions.
...
\caption{Summer evapotranspiration vs. 10cm soil water content from the CCE (dots) and the CESM2-LE (scatter density contours) across the three time periods. The relationship in the CCE closely follows the CESM2-LE pattern. The CCE exists at the dry extreme of the CESM2-LE, or even beyond, as in the 2090 scenario. }
...
Our CCE is able to reproduce the 2020 SWUS drought, despite known biases in the fully coupled CESM2.
This experiment demonstrates that antecedent soil moisture exacerbates drought severity, but not the precipitation amounts.
We likewise found only a small influence of climate change on precipitation volume during the 2020 drought in the CCE.
It showed a larger effect by the end of the 20th century, but subject to uncertainty due to potential circulation feedbacks.
...
While the 1850 and 2020 ensembles looked comparable, in 2090 the CCE features less precipitation, more moisture divergence, and drier near surface air than the driest corresponding years from the CESM2-LE.
This may indicate a propensity for CESM2 to mitigate moisture flux divergence via circulation feedbacks, but would need to be investigated more specifically.
...
The CCE approach is particularly useful for investigating the influence of antecedent conditions, because we are able to force drought in all simulations, but also sample a wide range of initial conditions

Ensemble boosting is useful and probably quite prudent
