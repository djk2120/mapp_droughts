...
Models tend to agree on trends in temperature, with associated reductions in soil moisture.
The forced responses of precipitation and runoff are more uncertain.
Precipitation is difficult to predict across this domain, due to complex orography and the dynamic nature of the North American Monsoon \cite{cook2020,pascale2017}.
...
However, the statistical basis can still become small when focusing explicitly on extreme quantiles, or when filtering for very specific circulation criteria. 
...
We opted to force our simulations with NOAA Extended Reconstructed Sea Surface Temperature V5 \cite{huang2017}. These are available at 2$^{\circ}$ resolution, which we regridded to the CESM2 nominal 1$^{\circ}$ resolution.
...
All other forcings (e.g. aerosols, CO$_2$, land use) follow the protocol of the CESM2-LE \cite{rodgers2021}, including emissions from CMIP6, that follow the historical and then the SSP3-7.0 emissions pathway.
